...
void man(char*name) // man means manual
{
	if(name==NULL) return;
	...
	else if(strcmp(name,"read")==0)
	{
		...
		printf("Usage: read File_name No_of_ Bytes_to_Read\n");
	}
...
int rm_File(char*name)
{
	...
	//free(UFDTArr[fd].ptrfilrtable->ptrinode->Buffer);
...
int LseekFile(int fd,int size,int from)
{
	...
	if((UFDTArr[fd].ptrfiletable->mode==READ)||(UFDTArr[fd].ptrfiletable->mode==READ+WRITE))
	{
		if(from==CURRENT)
		{
			...
			(UFDTArr[fd].ptrfiletable->readoffset)=(UFDTArr[fd].ptrfiletable->readoffset)=(UFDTArr[fd].ptrfiletable->readoffset)+size;
		}
		else if(from==START)
		{
